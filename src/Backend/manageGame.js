...
function doSwitch(player, toSwitch, setTop, topCard) {
  ...
  if (toSwitch[0].src === topCard.src) {
    for (let i = 0; i < 6; i++) {
      if (player[i].src === toSwitch[1].src) {
        pos = i;
      }
    }
  ...
  // Hand card first selected
    ...
    for (let i = 0; i < 6; i++) {
      if (player[i].src === toSwitch[0].src) {
        pos = i;
      }
    }
    let prev = JSON.parse(JSON.stringify(player[pos]));
...
function calcScore(a_hand) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (i === 0) {
      if (
        a_hand[i].val !== a_hand[1].val &&
        a_hand[i].val !== a_hand[3].val &&
        a_hand[i].val !== a_hand[4].val
      ) {
        if (a_hand[i].val === 11 || a_hand[i].val === 12) {
          tot += 10;
        } else {
          tot += a_hand[i].val;
        }
